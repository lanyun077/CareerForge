...
export default function RecordsPage() {
  ...
  function load() {
    setError('');
    request('/api/records').then((r) => r.json()).then((b) => b.ok ? setRecords(b.data as SessionSummary[]) : setError(b.message ?? '记录加载失败')).catch(() => setError('网络异常，请刷新重试'));
  }
  useEffect(load, [request, retryVersion]);

  async function remove(id: string) {
    if (!window.confirm('删除这次训练及报告？无其他训练使用时也会删除简历分析；推荐记录和导入岗位保留。')) return;
    setDeleting(id); setError('');
    try { const res = await request(`/api/records?id=${id}`, { method: 'DELETE' }); const body = await res.json(); if (!body.ok) setError(body.message ?? '删除失败'); else load(); } catch { setError('网络异常，删除失败'); } finally { setDeleting(''); }
  }

  const visible = useMemo(() => (records ?? []).filter((r) =>
    (filter === 'all' || r.status === filter) &&
    (!query.trim() || r.roleName.toLowerCase().includes(query.trim().toLowerCase())) &&
    (!date || new Date(r.startedAt).toLocaleDateString('sv-SE') === date)), [records, filter, query, date]);
  ...
  return <div className="space-y-5">
    ...
    <Link href="/recommendations" className="cf-button-secondary inline-block px-4 py-2 text-sm">推荐历史与清空</Link><p className="text-xs leading-5 text-slate-400">训练数据可由当前服务的内存模式或 PostgreSQL 保存，具体以首页状态为准；删除训练会移除报告及专属快照；简历分析仅在无其他训练引用时删除。推荐记录与导入岗位保留。</p>
  </div>;
}
